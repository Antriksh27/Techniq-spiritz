...
interface PipeSchematicBackgroundDarkProps {
  color?: string;
  opacity?: number;
}

export default function PipeSchematicBackgroundDark({ 
  color = "#FFFFFF", 
  opacity = 0.06 
}: PipeSchematicBackgroundDarkProps) {
    ...
    >
      ...
      >
        ...
        >
          <defs>
            <pattern
              id={`pipe-pattern-dark-${id.replace(/:/g, '')}`}
              x="0"
              y="0"
              width="250"
              height="250"
            ...
            >
              ...
              {/* VALVES */}
              <rect x="145" y="45" width="10" height="10" fill="#1C2B3A" stroke={color} strokeWidth="2" />
              <rect x="45" y="145" width="10" height="10" fill="#1C2B3A" stroke={color} strokeWidth="2" />
              <rect x="75" y="195" width="10" height="10" fill="#1C2B3A" stroke={color} strokeWidth="2" />

              {/* PRESSURE INDICATOR */}
              <circle cx="200" cy="125" r="9" fill="#1C2B3A" stroke={color} strokeWidth="2" />
          ...
          <rect width="100%" height="100%" fill={`url(#pipe-pattern-dark-${id.replace(/:/g, '')})`} />
        </svg>
      </motion.div>
    </div>
  );
